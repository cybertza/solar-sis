...
function MergeDataArray (json, queryValues) {
	...
	var s = JSON.parse(JSON.stringify(json), (key, value) => {
		...
		// check that we dont overflow so rather return
		if ( key == "" || value == undefined ) return value; // we dont want empty keys to mqtt, so lets just exit now.
		//client.publish('solarsis/pip/' + key, value.toString() );
		// if (value != undefined) client.publish(mqtttopic + key, value.toString() );
